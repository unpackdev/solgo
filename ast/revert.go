...
func (b *ASTBuilder) parseRevertStatement(
	...
	if ctx.CallArgumentList() != nil {
		for _, argumentCtx := range ctx.CallArgumentList().AllExpression() {
			statement.Arguments = append(
				statement.Arguments,
				b.parseExpression(
					sourceUnit, fnNode, bodyNode, nil, statement.Id, argumentCtx,
				),
			)
		}
	}

	statement.Expression = b.parseExpression(
		sourceUnit, fnNode, bodyNode, nil, statement.Id, ctx.Expression(),
	)
