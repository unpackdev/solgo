...
func (t *TypeName) Parse(unit *SourceUnit[Node], fnNode Node, pNode *Parameter, ctx parser.ITypeNameContext) {
	t.Id = t.GetNextID()
	t.Src = SrcNode{
		Id:          t.GetNextID(),
	...
	if ctx.ElementaryTypeName() != nil {
		...
		t.NodeType = pNode.NodeType

		typeCtx := ctx.ElementaryTypeName()
