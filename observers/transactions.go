	...
	for {
		...
		case entry := <-p.chTransactions:
			...
			if entry.SenderType == utils.ContractRecipient {
				group.Go(func() error {
					contract, err := p.contracts.Unpack(p.ctx, entry.Network, entry.Sender)
					if err != nil {
			...
			if entry.RecipientType == utils.ContractRecipient {
				group.Go(func() error {
					contract, err := p.contracts.Unpack(p.ctx, entry.Network, entry.Recipient)
					if err != nil {
			...
			if entry.TransactionType == utils.ContractCreationType {
				group.Go(func() error {
					contract, err := p.contracts.Unpack(p.ctx, entry.Network, entry.ContractAddress)
					if err != nil {
						/* zap.L().Error(
			...
			if len(entry.Receipt.Logs) > 0 {
				group.Go(func() error {
					for _, logEntry := range entry.Receipt.Logs {
						contract, err := p.contracts.Unpack(p.ctx, entry.Network, logEntry.Address)
						if err != nil {
							/* zap.L().Error(
								"Failed to unpack transaction contract",
								zap.Error(err),
...
func (p *TransactionsProcessor) decodeLogs(entry *TransactionEntry) error {
	...
	for _, log := range entry.Receipt.Logs {
		if logContract, ok := entry.LogContracts[log.Address]; ok {
			...
			fmt.Println("No address for log found....", log.Address.Hex())
		}
	}

	//spew.Dump(len(entry.Receipt.Logs), len(entry.Logs), entry.Logs)
