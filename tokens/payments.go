...
func (t *Token) Buy(ctx context.Context, client *clients.Client, exchangeType utils.ExchangeType, simulatorType utils.SimulatorType, spender *accounts.Account, baseToken *entities.Token, amount *entities.CurrencyAmount, atBlock *big.Int) (any, error) {
	...
	tradeResponse, err := uniV2.Buy(ctx, client, t.network, simulatorType, tokenBinding, spender, baseToken, t.GetEntity(), amount, block)
	if err != nil {
		return utils.ZeroAddress, fmt.Errorf(
...
func (t *Token) Sell(ctx context.Context, client *clients.Client, exchangeType utils.ExchangeType, simulatorType utils.SimulatorType, spender *accounts.Account, baseToken *entities.Token, amount *entities.CurrencyAmount, atBlock *big.Int) (any, error) {
	block := t.descriptor.BlockNumber
	if atBlock != nil {
		block = atBlock
	}
	...
	uniV2 := exchanges.ToUniswapV2(uniswapV2Exchange)
	tradeResponse, err := uniV2.Sell(ctx, client, t.network, simulatorType, tokenBinding, spender, t.GetEntity(), baseToken, amount, block)
	if err != nil {
		return utils.ZeroAddress, fmt.Errorf(
